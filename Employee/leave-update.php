...
// auth check
$user_id = $_SESSION['useremployee'];
if (isset($_SESSION['useremployee'])){
	 $task_id = $_GET['leave_id'];
...
if(isset($_POST['update_leave_info'])){
  $errmsg = $sql = "";
	...
	if(empty($leavetype ) || empty($leavedays) || empty($leavedate) || empty($empname) || empty($leave_type)|| empty($dept)|| empty($leavereason)|| empty($status)|| empty($replace))
{
	$errmsg.="One or more fields are empty...";
}
else{
	echo "makweli";
		$qle="UPDATE emp_leaves SET LeaveDays = '".$leavedays."', StartDate = '".$leavedate."', Reason= '".$leavereason."', Replaces = '".$replace."' WHERE id='".$task_id."'";
		if ($conn->query($qle) === TRUE) {
		echo "<center>";
		echo "<strong> Leave Updated Successful !</strong><br/><br/>";
		echo "</center>";
		header('location:clientnavi.php');
	}else {
...
$page_name="Edit Leave Request";
$sql = "SELECT * FROM emp_leaves WHERE id='$task_id' ";
  ...
  <div id="wrapper">
   ...
   <?php include "Navigation/sidebar.php";?>
			...
			$result2 = $conn->query($select);
				if ($result2->num_rows > 0) {
						while($row1 = $result2->fetch_assoc()) {
							...
							<tr><th>Designation : </th><td><?php echo $row1["Designation"]; ?></td></tr>
	...
	if($result3->num_rows > 0)
		...
		while($row3 = $result3->fetch_assoc())
			{
			echo "<h2>Request For A Leave for : ".$leave_type."</h2>";
			echo "<form action = '' method = 'post' style='margin-left: 36%;'>";
			echo "<table>";
			...
			echo "<input type = 'hidden' name = 'leavetype' value = '".$leave_type."'>";
			echo "<tr><th> *Starting Date : </th><td><input type = 'date' value='".$row["StartDate"]."' name = 'leavedate' class = 'textbox shadow selected' step = '1' ></td></tr>";
			echo "<tr><th> * No Of Leave Days : </th><td><input type = 'number' value='".$row["LeaveDays"]."' min = '1' name = 'leavedays' class = 'textbox shadow selected' step = '1'></td></tr>";
			echo "<tr><th> * Reason For Leave : </th><td><input type = 'text' value='".$row["Reason"]."' name = 'leavereason' class = 'textbox shadow selected' ></td></tr>";?>
			<tr><th> * Replacement : </th><td>
				<select name='employee' style="width: 100%;">
				<?//php EmpName != '".$row3['EmpName']."'?>
				<option value=""><?php echo $row['Replaces']; ?></option>
				<?php
				$sql3 = "SELECT EmpName,Dept FROM employees WHERE  Dept= '".$row3['Dept']."' AND EmpEmail != '".
				$row3['EmpEmail']."' AND EmpName !='".
				$row['Replaces']."'";
   ...
   ?>
	 <?php	
			...
			echo "<br/><tr><td><input type = 'submit' value = 'Update Leave' name='update_leave_info' class = 'login-button shadow' style='background-color: #303f9f; width:100%; margin-left:50%;'></td></tr>";
			echo "</form>";
			echo "<table>";
			$user = $_SESSION['useremployee'];
