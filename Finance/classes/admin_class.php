...
/* -------------admin_password_change------------*/
	...
	public function add_punch_in($data){
		...
		try{
			$add_progress = $this->db->prepare("INSERT INTO task_progress (atn_user_id, start_time) VALUES ('$user_id', '$punch_in_time') ");
			$add_progress->execute();

			header('Location: progress-info.php');
	...
	public function add_punch_out($data){
		...
		try{
			$update_user = $this->db->prepare("UPDATE task_progress SET end_time = :x, total_duration = :y WHERE aten_id = :id ");
